#!/usr/bin/env python3
"""Test suite for Morphy Greek POS tagger and dependency parser.
...
Run with:
    cd morphy && python -m pytest tests/test_morphy.py -x -v

For full tests including model inference (requires weights):
    python -m pytest tests/test_morphy.py -x -v --run-slow
"""
...
_HAS_GRC_WEIGHTS = (
    (_WEIGHTS_DIR / "grc" / "morphy_grc.pt").exists()
    or (_WEIGHTS_DIR / "grc" / "onnx" / "morphy_joint.onnx").exists()
)

_HAS_EL_WEIGHTS = (
    (_WEIGHTS_DIR / "el" / "morphy_el.pt").exists()
)
...
def morphy_grc():
    """Morphy instance for Ancient Greek (requires grc weights)."""
    from morphy import Morphy
    return Morphy(lang="grc", device="cpu")


@pytest.fixture(scope="module")
def morphy_grc_no_lemma():
    """Morphy instance for AG without lemmatization."""
    from morphy import Morphy
    return Morphy(lang="grc", device="cpu", lemmatize=False)


@pytest.fixture(scope="module")
def morphy_el():
    """Morphy instance for Modern Greek (requires el weights or HF download)."""
    from morphy import Morphy
    return Morphy(lang="el", device="cpu")
...
class TestTokenization:
    ...
    def test_batch_tokenize_basic(self):
        from morphy.tokenize import batch_tokenize
...
class TestLabels:
    ...
    def test_upos_labels_present(self):
        from morphy.labels import pos_labels
    ...
    def test_dp_labels_present(self):
        from morphy.labels import dp_labels
    ...
    def test_pos_properties_keys_are_valid_upos(self):
        from morphy.labels import pos_properties, pos_labels
    ...
    def test_el_label_counts(self):
        from morphy.labels import EL_POS_LABEL_COUNTS, EL_DP_LABEL_COUNT
...
class TestInitialization:
    """Test Morphy class construction and parameter handling."""

    def test_invalid_lang_raises(self):
        from morphy import Morphy
        with pytest.raises(ValueError, match="Unsupported language"):
            Morphy(lang="fr", device="cpu")

    def test_default_lang_is_el(self):
        """Verify default lang parameter is 'el'."""
        import inspect
        from morphy import Morphy
        sig = inspect.signature(Morphy.__init__)
    ...
    def test_valid_langs_accepted(self):
        ...
        for lang in ("el", "grc", "med"):
            try:
                Morphy(lang=lang, device="cpu", lemmatize=False)
    ...
    def test_dialect_parameter_stored(self):
        ...
        with patch.object(Morphy, "_init_grc", return_value=None):
            with patch.object(Morphy, "_init_lemmatizer", return_value=None):
                o = object.__new__(Morphy)
                o.device = "cpu"
                o.max_subwords = 2048
                o.lang = "grc"
                o._lemmatize = False
    ...
    @pytest.mark.slow
    def test_grc_init_creates_model(self, morphy_grc_no_lemma):
        """Verify AG initialization produces a working model."""
        assert morphy_grc_no_lemma.lang == "grc"
        assert morphy_grc_no_lemma.model is not None

    @pytest.mark.slow
    def test_lemmatize_default_true(self, morphy_grc):
        """Verify lemmatization is on by default when Dilemma is available."""
        if _has_dilemma():
            assert morphy_grc._lemmatize is True
        else:
            # Without Dilemma, _lemmatize gets set to False in _init_lemmatizer
            assert morphy_grc._lemmatize is False
...
class TestPOSTagging:
    ...
    def test_ag_sentence_produces_valid_upos(self, morphy_grc_no_lemma):
        """Tag an Ancient Greek sentence, verify all UPOS tags are valid."""
        results = morphy_grc_no_lemma.tag(["μῆνιν ἄειδε θεὰ Πηληϊάδεω Ἀχιλῆος"])
    ...
    def test_mg_sentence_produces_valid_upos(self, morphy_el):
        """Tag a Modern Greek sentence, verify all UPOS tags are valid."""
        results = morphy_el.tag(["Ο Αχιλλέας πολεμά"])
        assert len(results) == 1
        tokens = results[0]
        assert len(tokens) >= 3
        for tok in tokens:
...
class TestLemmatization:
    ...
    def test_lemmatizer_initialized(self, morphy_grc):
        """Verify Dilemma is initialized when lemmatize=True."""
        assert morphy_grc._lemmatizer is not None

    @pytest.mark.skipif(not _has_dilemma(), reason="Dilemma not installed")
    def test_output_includes_lemma(self, morphy_grc):
        """Verify that output tokens include a 'lemma' field."""
        results = morphy_grc.tag(["μῆνιν ἄειδε θεὰ Πηληϊάδεω Ἀχιλῆος"])
        for tok in results[0]:
    ...
    @pytest.mark.skipif(not _has_dilemma(), reason="Dilemma not installed")
    def test_dialect_parameter_flows_through(self, morphy_grc):
        """Verify dialect parameter is stored and available."""
        from morphy import Morphy
        model = Morphy(lang="grc", device="cpu", dialect="ionic")
    ...
    def test_lemma_cache_used(self):
        ...
        model = Morphy(lang="grc", device="cpu", lemma_cache=cache)
...
class TestBatchProcessing:
    ...
    def test_multiple_sentences(self, morphy_grc_no_lemma):
        ...
        results = morphy_grc_no_lemma.tag(sents)
        assert len(results) == 3
        for sent_result in results:
            assert isinstance(sent_result, list)
            assert len(sent_result) > 0

    def test_empty_input_returns_empty(self, morphy_grc_no_lemma):
        """Empty sentence list returns empty result list."""
        results = morphy_grc_no_lemma.tag([])
        assert results == []

    def test_single_string_input(self, morphy_grc_no_lemma):
        """A single string (not list) should be wrapped in a list."""
        results = morphy_grc_no_lemma.tag("μῆνιν ἄειδε θεά")
        assert len(results) == 1
        assert len(results[0]) >= 3

    def test_segment_text_flag(self, morphy_grc_no_lemma):
        """segment_text=True splits input string into sentences."""
        text = "μῆνιν ἄειδε θεά. ἄνδρα μοι ἔννεπε."
        results = morphy_grc_no_lemma.tag(text, segment_text=True)
...
class TestOutputStructure:
    ...
    def test_required_keys_present(self, morphy_grc_no_lemma):
    ...
    def test_raw_form_present(self, morphy_grc_no_lemma):
        """Token dicts should include raw_form (original polytonic form)."""
        results = morphy_grc_no_lemma.tag(["τὸν Ἀχιλλέα"])
        for tok in results[0]:
            assert "raw_form" in tok, f"Missing raw_form in token: {tok}"

    def test_feats_values_are_strings(self, morphy_grc_no_lemma):
        """Feature values should be strings, not integers or other types."""
        results = morphy_grc_no_lemma.tag(["ὁ ἄνθρωπος βαδίζει"])
        for tok in results[0]:
    ...
    def test_no_underscore_feats(self, morphy_grc_no_lemma):
    ...
    def test_head_zero_exists(self, morphy_grc_no_lemma):
...
class TestEdgeCases:
    ...
    def test_empty_string(self, morphy_grc_no_lemma):
    ...
    def test_single_word(self, morphy_grc_no_lemma):
        """Single word should produce exactly one token."""
        results = morphy_grc_no_lemma.tag("Ἀχιλλεύς")
        assert len(results) == 1
        assert len(results[0]) == 1

    def test_punctuation_only(self, morphy_grc_no_lemma):
    ...
    def test_mixed_punctuation_and_words(self, morphy_grc_no_lemma):
    ...
    def test_long_input(self, morphy_grc_no_lemma):
        ...
        results = morphy_grc_no_lemma.tag(long_sents)
...
class TestONNXParity:
    ...
    def onnx_available(self):
        onnx_path = _WEIGHTS_DIR / "grc" / "onnx" / "morphy_joint.onnx"
    ...
    def test_pos_parity(self, morphy_grc_no_lemma, onnx_available):
        """ONNX and PyTorch should produce identical UPOS tags."""
        from morphy import Morphy
        onnx_model = Morphy(lang="grc", device="cpu", checkpoint="onnx",
                            lemmatize=False)

        test_sent = "μῆνιν ἄειδε θεὰ Πηληϊάδεω Ἀχιλῆος"
        pt_results = morphy_grc_no_lemma.tag([test_sent])
    ...
    def test_deprel_parity(self, morphy_grc_no_lemma, onnx_available):
